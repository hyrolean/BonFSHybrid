BonDriver_uSUNpTV.dll MOD @ 2019/12/22
...
■オリジナル版から新たに追加した機能
  ...
  ・IDで個別指定できる機能を追加
    ...
    BonDriver_uSUNpTV_dev + ID(番号) + .dll のファイル名の記述方法で指定可能。
    BonDriver_uSUNpTV_dev0.dll ← 複数台接続された１番目のUS-3POUT
    BonDriver_uSUNpTV_dev1.dll ← 複数台接続された２番目のUS-3POUT
    BonDriver_uSUNpTV_dev2.dll ← 複数台接続された３番目のUS-3POUT
    上記以外の記述の場合は、空いている機器から順に探すオリジナルに準拠。
    BonDriver_uSUNpTV_A.dll などと記述するといままで通り自動でIDを割当てる。
  ...
  ・独自チャンネルファイル機能を追加
    ...
    サンプルとして BonDriver_uSUNpTV.ch.txt を添付。
...
2019/12/9 からの変更点
  ...
  ・BSチャンネルのストリーム切替のタイミングを再調整[12/21(ES)]
    (PSK復調レジスタのtmcerrビットクリア確定後に暫く待機してからtsidを切替える)

  ・キャッシュアロケータのメモリアクセスエラー不具合を修正[12/21(ES)]

2019/12/8 からの変更点

  ・USB BULK ストリームの通信パケットサイズを最大の47KBに変更
    ( 12/8修正分の64KBはそもそもコードを修正し忘れていて設定できていなかった。
      コードを見直した結果、47KBよりも大きい数値は設定できないっぽい。 )
...
2019/12/7 からの変更点
  ...
  ・チャンネルファイルの物理チャンネルにTSID(ID[整数])を記述できる項目を追加
  ・BSチャンネルのストリーム切替に稀に失敗することのある現象を修正
