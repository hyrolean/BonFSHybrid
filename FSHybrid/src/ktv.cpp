...
unsigned KtvDevice::DeMod_GetBER(int *pVal)
{
	unsigned flgs;
	uint8_t b_flg;
	unsigned val = DeMod_GetSequenceState();
	...
	b_flg = DeMod_Read(0x54);
	flgs = b_flg;
	if(b_flg & 1) pVal[0] = (DeMod_Read(0x55) << 16) | (DeMod_Read(0x56) << 8) | DeMod_Read(0x57);
	if(b_flg & 2) pVal[1] = (DeMod_Read(0x58) << 16) | (DeMod_Read(0x59) << 8) | DeMod_Read(0x5a);
	if(b_flg & 4) pVal[2] = (DeMod_Read(0x5b) << 16) | (DeMod_Read(0x5c) << 8) | DeMod_Read(0x5d);
	pVal[3] = 0x0fff * 204;
	pVal[4] = 0xffff * 204;
	pVal[5] = 0xffff * 204;
	DeMod_Write(0x53, 7 & ~b_flg);	DeMod_Write(0x53, 7);

	b_flg = DeMod_Read(0x62);
	flgs |= b_flg << 3;
	if(b_flg & 1) pVal[6] = (DeMod_Read(0x64) << 16) | (DeMod_Read(0x65) << 8) | DeMod_Read(0x66);
	if(b_flg & 2) pVal[7] = (DeMod_Read(0x67) << 16) | (DeMod_Read(0x68) << 8) | DeMod_Read(0x69);
	if(b_flg & 4) pVal[8] = (DeMod_Read(0x6a) << 16) | (DeMod_Read(0x6b) << 8) | DeMod_Read(0x6c);
	DeMod_Write(0x5f, 7 & ~b_flg);	DeMod_Write(0x5f, 7);

	DeMod_Write(0x50, 0xb8);
	b_flg = DeMod_Read(0x51);
	flgs |= b_flg << 6;
	if(b_flg & 1) {
		DeMod_Write(0x50, 0xb9);	val= DeMod_Read(0x51) << 8;
		DeMod_Write(0x50, 0xba);	val|= DeMod_Read(0x51);
		pVal[12] = val;
	}
	if(b_flg & 2) {
		DeMod_Write(0x50, 0xbb);	val= DeMod_Read(0x51) << 8;
		DeMod_Write(0x50, 0xbc);	val|= DeMod_Read(0x51);
		pVal[13] = val;
	}
	if(b_flg & 4) {
		DeMod_Write(0x50, 0xbd);	val= DeMod_Read(0x51) << 8;
		DeMod_Write(0x50, 0xbe);	val|= DeMod_Read(0x51);
		pVal[14] = val;
	}
	pVal[15] = 0x03ff;
	pVal[16] = 0x3fff;
	pVal[17] = 0x3fff;
	DeMod_Write(0x50, 0xb1);	DeMod_Write(0x51, b_flg);	DeMod_Write(0x51, 0);
...
#if 0
{ 791000, 0x08 }, { 809500, 0x07 }, { 828000, 0x06 }, { 846500, 0x05 },
{ 865000, 0x04 }
#endif
};
...
void Ktv1Device::ResetDeMod ()
{
	...
	// ŽóMLayer‚ðÝ’è
	tbl[1][1] = 0xb0U | (this->UserSettings & 0x7U);
	for ( unsigned i = 0; i < TBL_SZ( tbl ); i++ ) {
		DeMod_Write(tbl[i][0], tbl[i][1]);
	}
}
...
void Ktv2Device::InitDeMod()
{
	static uint8_t tbl[][2] = {
		...
		,{0xec, 0xff} //split?
		,{0xec, 0xff}
		// Set IRQ Channel Search End Off
		,{0xee, 0x03} //split?
		,{0xee, 0x02}
		// Set Demod Tuning Mode
		,{0x39, 0x01}
		,{0x3b, 0x05},{0x3c, 0x01}
		,{0x3a, 0x15},{0x3a, 0x05}
		,{0x71, 0x00}
